...
- Sessions: list + per-session model/thinking/fast/verbose/reasoning overrides, detached-session inspect/detail, transcript previews, detached follow-up, and foreground reattach (`sessions.list`, `sessions.patch`, `sessions.inspect`, `sessions.send`, `sessions.reattach`)
